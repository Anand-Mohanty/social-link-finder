...
# Maximum number of URLs/usernames accepted per submission. Sequential
# processing plus per-target network round trips means a big batch takes
# a while and hammers a lot of external sites back-to-back; this cap keeps
# a single request from running for many minutes or looking like abuse.
MAX_BATCH_SIZE = 25
...
def parse_entries(pasted_text: str, uploaded_file) -> list:
    """Combines pasted textarea lines and an uploaded .txt/.csv file into
    one deduplicated, order-preserving list of raw entries (URLs or bare
    ...
    if uploaded_file and uploaded_file.filename:
        ...
        if uploaded_file.filename.lower().endswith(".csv"):
            ...
            for row in reader:
                ...
                if candidate and candidate.lower() not in ("url", "username", "urls", "usernames"):
                    entries.append(candidate)
        else:  # .txt or unrecognized extension: treat as one entry per line
...
def process_entry(raw_entry: str) -> dict:
    """Runs the same single-target lookup logic as the original tool
    against one line of input, returning a self-contained result record."""
    record = {
        "input": raw_entry,
        "type": None,
        "target": None,
    ...
    raw = raw_entry.strip()
    if not raw:
        record["error"] = "Empty entry."
        return record

    if looks_like_username(raw):
        record["type"] = "username"
        username = raw.lstrip("@")
        record["target"] = username
        matches = probe_username(username)
        record["other_matches"] = matches
        if not matches:
            record["error"] = f'No profiles found for username "{username}".'
        return record
    ...
    if not fetch(target):
        record["error"] = f"Couldn't reach {target} (site may block automated requests)."
        return record

    data = find_socials(target)
    record["results"] = data["results"]
    record["pages_checked"] = data["pages_checked"]

    guess = best_guess_username(data["results"], target)
    if guess and USERNAME_RE.match(guess):
        already_linked = {u for urls in data["results"].values() for u in urls}
        probed = probe_username(guess)
        other = {platform: url for platform, url in probed.items() if url not in already_linked}
        record["other_matches"] = other

    if not record["results"] and not record["other_matches"]:
        record["error"] = "No social links found, and no matching username found elsewhere."

    return record


# In-memory cache of the most recent batch run, used by the export routes.
# This app is designed for single-user local use (see README) so a plain
# module-level variable is enough — no need for sessions or a database.
_last_batch_results = []
...
def index():
    ...
    if request.method == "POST":
        pasted_text = request.form.get("urls", "")
        uploaded_file = request.files.get("file")

        entries = parse_entries(pasted_text, uploaded_file)

        if not entries:
            error = "Paste at least one URL/username, one per line, or upload a .txt/.csv file."
        elif len(entries) > MAX_BATCH_SIZE:
            error = f"Please limit a batch to {MAX_BATCH_SIZE} entries at a time (you submitted {len(entries)})."
        else:
            batch_results = []
            for entry in entries:  # sequential by design: one target at a time
                batch_results.append(process_entry(entry))
...
def export_json():
    ...
    payload = json.dumps(_last_batch_results, indent=2).encode("utf-8")
    ...
    return send_file(
        ...
        as_attachment=True,
        download_name="social_finder_results.json",
    )
...
def export_csv():
    ...
    writer.writerow(["input", "type", "target", "platform", "profile_url", "source", "error"])

    for record in _last_batch_results:
        wrote_row = False
        for platform, urls in (record.get("results") or {}).items():
            for u in urls:
                writer.writerow([record["input"], record["type"], record["target"], platform, u, "scraped", record.get("error") or ""])
                wrote_row = True
        for platform, u in (record.get("other_matches") or {}).items():
            writer.writerow([record["input"], record["type"], record["target"], platform, u, "username-probe", record.get("error") or ""])
            wrote_row = True
        if not wrote_row:
            writer.writerow([record["input"], record["type"], record["target"], "", "", "", record.get("error") or ""])

    mem_buf = io.BytesIO(text_buf.getvalue().encode("utf-8"))
    mem_buf.seek(0)
    return send_file(
        mem_buf,
        mimetype="text/csv",
        as_attachment=True,
        download_name="social_finder_results.csv",
    )


if __name__ == "__main__":
    # 127.0.0.1 keeps this reachable only from this machine. Do not change
    # to 0.0.0.0 without also turning debug off — see README for why.
